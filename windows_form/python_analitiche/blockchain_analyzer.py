...
class BlockchainStats:
    ...
    def calcola_ricchi_della_rete(self):
        bilanci = {}
        utxo = {}  # (txid, index) -> (indirizzo, valore)
        ...
        for blocco in reversed(self.data):
            for tx in blocco.get('transactions', []):
                ...
                for vin in tx.get('vin', []):
                    ref = (vin.get('txid'), vin.get('vout_index'))
                    if ref in utxo:
                        indirizzo, valore = utxo.pop(ref)
                        bilanci[indirizzo] -= valore

                # Aggiungi output 
                for i, output in enumerate(tx.get('vout', [])):
                    indirizzo = output.get('pubkey_hash')
                    valore = float(output.get('value', 0))

                    utxo[(txid, i)] = (indirizzo, valore)
